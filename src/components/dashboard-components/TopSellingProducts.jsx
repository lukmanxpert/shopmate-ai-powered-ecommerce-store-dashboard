  ...
  return <>
    <div className="bg-white rounded-xl p-6 shadow-md overflow-x-auto xl:col-span-2 max-h-[440px] scrollbar-hide">
      ...
      <table className="min-w-[600px] w-full text-sm text-left">
        ...
        <tbody>
          {
            topSellingProducts.length > 0 && topSellingProducts.map((el, index) => {
              return (
                <tr key={index} className="border-b hover:bg-gray-50 transition">
                  <td className="px-4 py-3">
                    <img src={el.image} alt={el.name} className="w-12 h-12 object-cover rounded-md" />
                  </td>
                  <td className="px-4 py-3 font-medium">{el.name}</td>
                  <td className="px-4 py-3 text-gray-600">{el.category}</td>
                  <td className="px-4 py-3 font-semibold">{el.total_sold}</td>
                  <td className="px-4 py-3 text-yellow-500 font-semibold">{el.ratings}</td>
                </tr>
              )
            })
          }
